...
void handle_pseudo_cmd(FILE *file, int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,
	...
	fprintf(file, "0x%08x\n",
		cmd2code(lineNum, cmdNum, "lui", pArg1, pArg2, pArg3, pLabel));
	...
	temp1[i] = '\0';
	sprintf(temp2, "%d", (handle_label_imm(pArg2, pLabel, cmdNum, lineNum)) & 0xFFF);
	
	fprintf(file,"0x%08x\n",
		cmd2code(lineNum, cmdNum, "addi", pArg1, temp1, temp2, pLabel));		
...
int cmd2code(int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,
	...
	if (!strncmp(pOpcode, "addi", 4) && strlen(pOpcode) == 4) {
		...
		printf("Lab 2-1 assignment: ADDI instruction\n");
	...
	// Integer Register-Register Operations
		...
		printf("Lab2-1 assignment: ADD instruction\n");
	...
	// Unconditional Jumps
		...
		int offset;
		offset = handle_label_imm(pArg2,pLabel,cmdNum,lineNum);	
		code += (offset<<20);
